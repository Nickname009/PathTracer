...
public class Geometry {
	...
	public void add( Geometry O,Geometry P) {
		this.x = O.getX() + P.getX();
		this.y = O.getY() + P.getY();
		this.z = O.getZ() + P.getZ();
	}
	...
	public Direction add( Direction d ,Geometry O, Geometry P) {
	...
	public Geometry addG(Geometry O, Geometry P) {
	...
	public static Geometry sub(Geometry O, Geometry P) {
		return new Geometry(O.getX() - P.getX(), O.getY() - P.getY(), O.getZ() - P.getZ());
	}
	...
	public static Direction sub( Direction d ,Geometry O, Geometry P) {
		d.setX(O.getX() - P.getX());
		d.setY(O.getY() - P.getY());
		d.setZ(O.getZ() - P.getZ());
		return d;
...
/*	public double[][] crossMM(Matrix4d change,Matrix4d change2) {
		double m[][]= new double[1][4];
		double matrix[][]= change.getMatrix();
		double matrix2[][]= change2.getMatrix();
		m[0][0]=(matrix[0][0]*p.getX()+matrixChange[0][1]*p.getY()+matrixChange[0][2]*p.getZ()+matrixChange[0][3]*1);
		m[0][1]=(matrixChange[1][0]*p.getX()+matrixChange[1][1]*p.getY()+matrixChange[1][2]*p.getZ()+matrixChange[1][3]*1);
		m[0][2]=(matrixChange[2][0]*p.getX()+matrixChange[2][1]*p.getY()+matrixChange[2][2]*p.getZ()+matrixChange[2][3]*1);
		m[0][3]=(matrixChange[3][0]*p.getX()+matrixChange[3][1]*p.getY()+matrixChange[3][2]*p.getZ()+matrixChange[3][3]*1);
		System.out.println("x: " +  m[0][0] +"y: " +  m[0][1] +"z: " + m[0][2]+"w: " + m[0][3]);
